...
public class HowMuchProcessor implements SentenceProcessor {
    @Override
    public void process(String sentence) {
        Pattern pattern = Pattern.compile(SentenceType.HOW_MUCH.getPattern());
        Matcher matcher = pattern.matcher(sentence);
        while (matcher.find()) {
            String galaxyDigit = matcher.group(1);
            try {
                int number = GalaxyDigitsConverter.convertGalaxyDigitToNumber(galaxyDigit);
                System.out.println(String.format("%s is %d", galaxyDigit, number));
